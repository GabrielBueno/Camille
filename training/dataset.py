from labels import get_dir_label
...
class WikiArtDataCollection():
    ...
    def __init__(self, training_transforms, validation_transforms, validation_percent):
        self.validation_percent = validation_percent

        # Init datasets
        dirs = [path for path in os.scandir(self.ROOT) if path.is_dir()]
        imgs = [(file.path, get_dir_label(dir.name)) for dir in dirs for file in os.scandir(dir.path)]
