...
O trabalho original de Rosenblatt define como função de ativação para o \emph{perceptron} uma função binária $p(x)$ que produz $0$ ou $1$ caso $x$ seja ou não positivo.
...
\begin{equation}
\frac{1}{n} \displaystyle\sum_{i=0}^{n} (y_i - \hat{y}_i)^2,
\end{equation}

Onde $n$ é a quantidade de entradas utilizadas, $y_i$ é o valor previsto pela rede neural, e $\hat{y}_i$ é o valor correto que a rede deveria ter previsto.
